...
public class StepDefination {
	...
	public void verify_the_book_name_authorname_price(DataTable dataTable) throws Throwable {
		...
		((JavascriptExecutor) dv).executeScript("window.focus();");

		String bookname = dv.findElement(By.xpath(ObjectRepository.ResourceXPath.prepareBookname(Bookname))).getText();
		Thread.sleep(2000);
		ArrayList<String> tabs2 = new ArrayList<String> (dv.getWindowHandles());
		dv.switchTo().window(tabs2.get(1));
		String bookauth = dv.findElement(By.xpath(ObjectRepository.ResourceXPath.targetBookAuthorNAME)).getText();
		String bookprice = dv.findElement(By.xpath(ObjectRepository.ResourceXPath.targetBookPRICE)).getText().substring(2);

		//System.out.println("********************************" + bookname + " "  + " " + bookauth+ " " + bookprice);
		assertEquals(Bookname.trim(), bookname.trim());
		assertEquals(Bookauthor.trim(), bookauth.trim());
		assertEquals(Bookprice.trim(), Bookprice.trim());
	}
	
	@And("^Write it in excel file$")
    public void write_it_in_excel_file(DataTable dataTable) throws Throwable {
//		List<List<String>> obj = dataTable.raw();
//		String Bookname = obj.get(0).get(0);
//		String Bookauthor = obj.get(0).get(1);
//		String Bookprice = obj.get(0).get(2);
...
//        Connection connection = fillo.getConnection("E:\\ME as QA\\AmazonCucumber\\AmazonWebportal\\target\\ExcelReport.xlsx");
//        String qury = "INSERT INTO sheet4(Name,Author,Price) VALUES('"+ Bookname +"','"+ Bookauthor +"','"+ Bookprice+"')";
//        connection.executeQuery(qury);
//        connection.close();
    }
}
